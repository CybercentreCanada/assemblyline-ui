...
var app = angular.module('app', ['utils', 'search', 'socket-io', 'ngAnimate', 'ui.bootstrap'])
    ...
    .controller('ALController', function ($scope, $http, $timeout, mySocket) {
        ...
        $scope.hasLoadedVMs = function(host){
            if (host === null || host === undefined){
                return false;
            }
            if (host.hasOwnProperty('last_hb')){
                if (host.last_hb.hasOwnProperty('vmm')){
                    if ($scope.getKeys(host.last_hb.vmm).length!==0){
                        return true;
                    }
                }
            }
        ...
        $scope.hasLoadedServices = function(host){
            if (host === null || host === undefined){
                return false;
            }
            if (host.hasOwnProperty('last_hb')){
                if (host.last_hb.hasOwnProperty('services')){
                    if (host.last_hb.services.hasOwnProperty('details')) {
                        if ($scope.getKeys(host.last_hb.services.details).length !== 0) {
                            return true;
                        }
                    }
                }
        ...
        $scope.get_node_state = function (node) {
            ...
            /*
             ...
             'up': host.controller=='up'&&host.hostagent_status=='up',
        ...
        $scope.$on('socket:OrchestratorHeartbeat', function (event, data) {
            ...
            try {
                ...
                $scope.last_msg_time = cur_time;
                $scope.host_array[mac]['orchestrator_status'] = 'up';
                $scope.host_array[mac]['orchestrator_timer'] = cur_time;
                $scope.host_array[mac]['orchestrator'] = data.body;

                if ($scope.host_array[mac]['resources'] == null) {
                    $scope.host_array[mac]['resources'] = data.body.resources;
                    $scope.host_array[mac]['orchestrator_stat'] = {count: 1};
                }
                else {
                ...
                if ($scope.debug) {
                    data['ui_time'] = cur_time;
                    //console.log('Socket-IO::OrchestratorHeartbeat message', data);
                }
                $timeout(function () {
                    if (($('#myModal').data('bs.modal') || {}).isShown){
                        if ($scope.cpu_gauge === undefined || $scope.cpu_gauge === null){
                            $scope.cpu_gauge = create_gauge('cpu');
                            $scope.cpu_gauge.render();
                        }
                        if ($scope.ram_gauge === undefined || $scope.ram_gauge === null){
                            $scope.ram_gauge = create_gauge('ram');
                            $scope.ram_gauge.render();
                        }
                        $scope.cpu_gauge.moveTo(Math.min($scope.host_array[mac]['orchestrator']['resources'].cores.used/$scope.host_array[mac]['orchestrator']['resources'].cores.total, 1));
                        $scope.ram_gauge.moveTo(Math.min($scope.host_array[mac]['orchestrator']['resources'].memory.used/$scope.host_array[mac]['orchestrator']['resources'].memory.total, 1));
                    }
                }, 0)

            }
            catch (e) {
                console.log('Socket-IO::OrchestratorHeartbeat [ERROR] Invalid message', data, e);
            }
        ...
        $scope.doSearch = function (row) {
            ...
            if (row.resources != null) {
                return ((re.test(row.mac_address) || re.test(row.hostname) || re.test(row.parent_name) || re.test(row.parent_mac) || re.test(row.ip) || re.test(row.resources['service.list']) || re.test(row.resources['vm.list'])));
            }
            else {
                return ((re.test(row.mac_address) || re.test(row.hostname) || re.test(row.parent_name) || re.test(row.parent_mac) || re.test(row.ip)));
            }
        };
        ...
        $("#myModal").on('shown.bs.modal', function(){
            if ($scope.current_host.orchestrator !== undefined){
                if ($scope.cpu_gauge === undefined || $scope.cpu_gauge === null){
                    $scope.cpu_gauge = create_gauge('cpu');
                    $scope.cpu_gauge.render();
                }
                ...
                $scope.cpu_gauge.moveTo(Math.min($scope.current_host.orchestrator.resources.cores.used/$scope.current_host.orchestrator.resources.cores.total, 1));
                $scope.ram_gauge.moveTo(Math.min($scope.current_host.orchestrator.resources.memory.used/$scope.current_host.orchestrator.resources.memory.total, 1));
            }
        });
                ...
                if ((item['orchestrator_timer'] + timer - cur_time) < 0) {
                    if (!mySocket.connected()) {
                        ...
                        if (item['orchestrator_status'] == 'init') {
                            item['orchestrator_status'] = 'grace-period';
                        }
                        else {
                            item['orchestrator_status'] = 'down';
                        }
                    }
                }
                ...
                .success(function (data) {
                    ...
                    for (var h in temp_host_list) {
                        ...
                        if (!(item.mac_address in $scope.host_array)) {
                            ...
                            item['orchestrator_status'] = 'init';
                            item['orchestrator_timer'] = 0;
                            item['resources'] = null;
