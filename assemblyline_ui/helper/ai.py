...
def summarize_code_snippet(code):
    ...
    data = {
        "max_tokens": config.ui.ai.max_tokens,
        "messages": [
            {"role": "system", "content": config.ui.ai.code_system_message},
            {"role": "user", "content": yaml.dump(code)},
        ],
        "model": config.ui.ai.model_name,
        "stream": False
    }
    ...
    if not resp.ok:
        raise AiApiException(f"An exception occured while trying to sumarize the code snippet with AI. ({resp.json()})")
