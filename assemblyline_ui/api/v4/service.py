...
event_sender = EventSender(
    "changes.services", host=config.core.redis.nonpersistent.host, port=config.core.redis.nonpersistent.port
)

root_event_sender = EventSender(
    ...
    for delta in delta_list:
        if delta["name"] == source["name"]:
            ...
            if delta["default_classification"] != source["default_classification"]:
                ...
                STORAGE.signature.update_by_query(
                    query=f'source:"{source["name"]}"', operations=[("SET", "classification", class_norm)]
                )
...
def get_service_stats(service_name, version=None, max_docs=500):
    ...
    if version:
        try:
            ...
            if "dev" not in build:
                filters.append(
                    f"response.service_version:{framework}.{major}.{minor}.{build} OR "
    ...
    if len(res["items"]) == 0:
        ...
        data = {
            "error": errors,
            "file": {"extracted": {"avg": 0, "max": 0, "min": 0}, "supplementary": {"avg": 0, "max": 0, "min": 0}},
            "heuristic": heuristics,
            "result": {"count": 0, "score": {"avg": 0, "distribution": {"0": 0, "500": 0}, "max": 0, "min": 0}},
        ...
        score_stats["distribution"] = STORAGE.result.histogram(
            "result.score", start=min_score, end=max_score, gap=gap, mincount=0, query=query, filters=filters
        )
        ...
        extracted = {
            k: v or 0
            for k, v in STORAGE.result.stats(
                "response.extracted.length",
                query=query,
                filters=filters,
        ...
        data = {
            "service": {"name": service_name},
            "error": errors,
            "file": {"extracted": extracted, "supplementary": supplementary},
            "heuristic": heuristics,
            "result": {"count": result_count, "score": score_stats},
        }
        ...
        if source not in new_sources:
            ...
            if not source_exists(new_sources, source):
                removed_sources[source["name"]] = (
                    STORAGE.signature.delete_by_query(f'type:"{service_name.lower()}" AND source:"{source["name"]}"')
                    != 0
                )
                service_updates = Hash(
                    f"service-updates-{service_name}",
                    config.core.redis.persistent.host,
...
def add_service(**_):
    ...
    try:
        ...
        event_sender.send(service.name, {"operation": Operation.Added, "name": service.name})
        ...
        return make_api_response(dict(service_name=service.name, new_heuristics=new_heuristics))
...
def backup(**_):
    ...
    full = request.args["full"] if "full" in request.args else False

    for service in STORAGE.service_delta.stream_search("*:*", fl="id,version", as_obj=False, item_buffer_size=1000):
        name = service["id"]
        service_output = {"config": STORAGE.service_delta.get(name, as_obj=False), "versions": {}}
        if full:
    ...
    return make_file_response(
        out, name=f"{config.ui.fqdn}_service_backup.yml", size=len(out), content_type="application/json"
    )
...
def restore(**_):
    ...
    try:
        ...
        for service_name, service in backup["data"].items():
            ...
            # Notify components watching for service config changes
            event_sender.send(
                service_name, {"operation": Operation.Added if old_service else Operation.Removed, "name": service_name}
            )
...
def list_all_services(**_):
    ...
    resp = [
        ...
        if Classification.is_accessible(user["classification"], x.get("classification", Classification.UNRESTRICTED))
    ]
...
def remove_service(servicename, **_):
    ...
    if svc:
        ...
        # Notify components watching for service config changes
        event_sender.send(servicename, {"operation": Operation.Removed, "name": servicename})
        ...
        return make_api_response({"success": False}, err=f"Service {servicename} does not exist", status_code=404)
...
def set_service(servicename, **_):
    ...
    if success:
        event_sender.send(servicename, {"operation": Operation.Modified, "name": servicename})

    return make_api_response({"success": success, "removed_sources": removed_sources})
    ...
    try:
        ...
        for service in services:
            if service["name"] not in installed_services:
                image = service["image"]
                install_data = {"image": f"${{REGISTRY}}{image}" if not image.startswith("$") else image}
...
def update_service(**_):
    ...
    if STORAGE.service.get_if_exists(service_key):
        operations = [
            (STORAGE.service_delta.UPDATE_SET, "version", data["update_data"]["latest_tag"].replace("stable", ""))
        ]
        if STORAGE.service_delta.update(data["name"], operations):
            event_sender.send(data["name"], {"operation": Operation.Modified, "name": data["name"]})
        ...
        if srv_update_available and not srv_updating:
            ...
            if STORAGE.service.get_if_exists(f"{name}_{clean_latest_tag}"):
                ...
                if STORAGE.service_delta.update(name, operations):
                    event_sender.send(name, {"operation": Operation.Modified, "name": name})
